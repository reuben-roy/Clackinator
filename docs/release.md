...
1. Configure signing in Xcode or the CI environment for the `ClackinatorDirect` target.
...
6. Copy the rendered cask into your `homebrew-clackinator` tap repository under `Casks/clackinator.rb`.
...
1. Open the generated Xcode project and select the `ClackinatorAppStore` target.
2. Set the appropriate team, provisioning, and archive signing settings.
3. Run the validation gate from [docs/app-store-validation.md](app-store-validation.md).
